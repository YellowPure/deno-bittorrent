...
export class Worker {
  ...
  async connect() {
    ...
    const conn = await connectWithTimeout(
      { hostname: this.peer.ip, port: this.peer.port },
      5000,
      );
      this.conn = conn;
      // handshake
    ...
    const receivedInfoHash = buf.slice(
      protocolIDLength + 8,
      protocolIDLength + 8 + 20,
    );
    if (!typedArraysAreEqual(receivedInfoHash, infoHash)) {
      throw new Error(
        `unmatched info hash, expected ${infoHash}, got ${receivedInfoHash}`,
      );
    }
  ...
  async start() {
    try {
      ...
      this.error(`could not connect to ${this.peerStr}: ${error}`);
      this.onFailure();
      return;
    }

    this.info(`connect established`);
    ...
    while (true) {
      ...
      if (!this.bitfield.hasPiece(pieceIndex)) {
        ...
        if (work.ttl <= 0) {
          throw new Error(
            `could not find peers that have this piece#${pieceIndex}`,
          );
        }
      ...
      try {
        ...
        this.info(`pieces#${pieceIndex} downloaded`)

        this.onResult(result)
      } catch (error) {
        this.error(`failed to download pieces#${pieceIndex}: ${error}`)
        this.putBackWork(work)
      }
    }
  ...
  async sendMessage(message: Message) {
...

    {
      ...
      const length = (message.payload?.length ?? 0) + 1;
