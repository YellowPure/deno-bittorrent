import { assert, decodeBencode, encodeBencode, log } from "./deps.ts";
...
const parseTorrentFile = (filepath: string): Torrent => {
  ...
  logDebug(parsed);
  assert(parsed.info.length != null, "only supply single file torrent");
  ...
  assert(
    ...
    `expect ${Math.ceil(totalLength / pieceLength)}, got ${pieceCount}`,
  );
...
await log.setup({
  handlers: {
    workerFmt: new log.handlers.ConsoleHandler("DEBUG", {
      formatter: (logRecord) => {
        return `${logRecord.levelName} [worker.${
          logRecord.args[0]
        }] ${logRecord.msg}`;
      },
    }),
